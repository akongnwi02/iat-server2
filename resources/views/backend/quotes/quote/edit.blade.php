...
@section('content')
    {{ html()->modelForm($quote, 'PUT', route('admin.quotes.quote.update', $quote))->class('form-horizontal')->open() }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">@lang('labels.backend.quote.quote.edit')</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
                ...
                <div class="col">
                    ...
                    <div id="POItablediv">
                        ...
                        <table class="table table-responsive table-borderless" id="POITable">
                            ...
                            <tbody>
                            @foreach($quote->inventories as $key => $value)
                                <tr>
                                    <td>
                                        <select id="material" style="min-width:100px" name="inventories[{{$key}}][inventory_id]" class="form-control" required>
                                            <option></option>
                                            @foreach($inventories as $inventory)
                                                <option value="{{ $inventory->uuid }}" {{($value->uuid == $inventory->uuid) ? "selected":""}}>{{ $inventory->{'name_' . $current_locale} }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td><input id="unit_cot" style="min-width:100px" type="number" name="inventories[{{$key}}][unit_cost]" step="0.01" class="form-control" min="0" value="{{$value->pivot->unit_cost}}" required/></td>
                                    <td><input id="quantity" style="min-width:100px" type="number" name="inventories[{{$key}}][quantity]" step="1" class="form-control" min="0" value="{{$value->pivot->quantity}}" required/></td>
